...
func TestWebServer(t *testing.T) {
	...
	for key := range keys {
		...
		_, err := http.Get(fmt.Sprintf(ts1.URL+"/set?key=%s&value=value-%s&bucketName=default", key, key))
		if err != nil {
	...
	for key := range keys {
		// Send all to first shard to test redirects.
		resp, err := http.Get(fmt.Sprintf(ts1.URL+"/get?key=%s&bucketName=default", key))
		if err != nil {
	...
	value1, err := db1.GetKey("USA", "default")
	if err != nil {
	...
	value2, err := db2.GetKey("Soviet", "default")
	if err != nil {
